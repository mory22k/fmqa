...
def sa_ising(J, h, return_multiple=False, **kwargs) -> torch.Tensor:
    """
    ...
    if return_multiple:
        return torch.stack([torch.from_numpy(sample).to(J.device) for sample in record.sample])
    return torch.from_numpy(best_sample).to(J.device)

def sa_qubo(Q, return_multiple=False, **kwargs):
    """
    ...
    best_sample = record[0].sample
    if return_multiple:
        return torch.stack([torch.from_numpy(sample).to(Q.device) for sample in record.sample])
